...
async def confirm_user(confirmation_uuid: UUID, db: AsyncSession = Depends(get_db_session)):
    result = await db.execute(select(User).where(User.confirmation_uuid == confirmation_uuid))
    ...
    if user.confirmed:
        return JSONResponse(status_code=status.HTTP_208_ALREADY_REPORTED, content={"message": "User already confirmed"})
    ...
    return JSONResponse(status_code=status.HTTP_202_ACCEPTED, content={"message": f"User {user.username} confirmed"})


@router.post("/auth")
async def authorize_user(user: UserAuthorize, db: AsyncSession = Depends(get_db_session)):
    result = await db.execute(select(User).where(User.username == user.username))
